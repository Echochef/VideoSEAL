...
class MultiTurnWorkflow(TimingTrackingMixin, Workflow):
    ...
    async def run(self, task: dict, uid: str, **kwargs) -> Episode | None:
        ...
        for _ in range(1, self.max_steps + 1):
            ...
            # Agent.update_from_model may return either a plain action object
            # (e.g., list/dict/str) or an Action dataclass wrapper. The
            # ToolEnvironment expects the raw underlying data (list/str/dict),
            # so unwrap when needed.
            action_obj = self.agent.update_from_model(response)
            try:
                from rllm.agents.agent import Action  # local import to avoid cycles
            except Exception:  # pragma: no cover
                Action = None  # type: ignore[assignment]

            if Action is not None and isinstance(action_obj, Action):
                env_action = action_obj.action
            else:
                env_action = action_obj

            next_obs, reward, done, info = await self.timed_env_call(self.env.step, env_action)
