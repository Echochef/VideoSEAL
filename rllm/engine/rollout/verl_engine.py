...
class VerlEngine(RolloutEngine):
    def __init__(self, config, rollout_manager, tokenizer, processor=None, **kwargs):
        ...
        # Rollout-time limits should follow actor_rollout_ref.rollout.* so users can decouple
        # PPO tensorization (data.max_*) from multi-turn context/step generation budgets.
        # PPO still pads/truncates using data.max_prompt_length/data.max_response_length downstream.
        self.max_prompt_length = int(config.actor_rollout_ref.rollout.get("prompt_length", config.data.max_prompt_length))
        self.max_response_length = int(
            config.actor_rollout_ref.rollout.get("response_length", config.data.max_response_length)
        )
