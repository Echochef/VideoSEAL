...
class AgentWorkflowEngine:
    ...
    def transform_results_for_verl(self, episodes: list[Episode], task_ids: np.ndarray) -> "DataProto":
        ...
        for i, episode in enumerate(episodes):
            ...
            for trajectory in episode.trajectories:
                ...
                if not self.config.rllm.stepwise_advantage.enable:
                    ...
                    # Aggregate tool transcript/info over the whole trajectory for this sample.
                    # NOTE: Our tool environment writes `tool_outputs` for calls issued at step i
                    # into the *next* step's observation (i+1). The original agentrllm engine
                    # assumes same-step alignment. To stay compatible with both layouts, we:
                    #   - collect non-finish calls at step i
                    #   - look for matching `tool_outputs` either in step i+1 (preferred, our env)
                    #     or fall back to step i (agentrllm-style env) by matching call ids.
                    try:
                        tool_total = 0
                        transcript = []
                        ...
                        for idx, st in enumerate(steps):
                            if getattr(st, "model_response", None):
                                transcript.append({"assistant_response": st.model_response})

                            # Collect tool calls for this step
                            all_calls: list[dict] = []
                            non_finish_calls: list[dict] = []
                            non_finish_call_ids: list[str] = []
                            for tc in (st.action or []):
                                if not isinstance(tc, dict):
                                    continue
                                fn = (tc or {}).get("function", {})
                                name = fn.get("name")
                                args = fn.get("arguments")
                                # Best-effort JSON parse if arguments is a stringified dict
                                if isinstance(args, str):
                                    try:
                                        ...
                                        pass
                                call_entry = {"name": name, "arguments": args}
                                all_calls.append(call_entry)
                                if name != "finish":
                                    non_finish_calls.append(call_entry)
                                    tc_id = tc.get("id")
                                    if isinstance(tc_id, str):
                                        non_finish_call_ids.append(tc_id)

                            # Determine aligned tool outputs for non-finish calls at this step.
                            outputs = {}
                            if non_finish_call_ids:
                                # Prefer next-step observation (our env: obs[i+1].tool_outputs
                                # corresponds to calls at step i).
                                if idx + 1 < n_steps:
                                    next_obs = getattr(steps[idx + 1], "observation", None)
                                    if isinstance(next_obs, dict) and "tool_outputs" in next_obs:
                                        candidate = next_obs.get("tool_outputs") or {}
                                        if isinstance(candidate, dict) and set(candidate.keys()) & set(non_finish_call_ids):
                                            outputs = candidate

                                # Fallback: some environments record tool_outputs on the same step
                                if not outputs:
                                    obs = getattr(st, "observation", None)
                                    if isinstance(obs, dict) and "tool_outputs" in obs:
                                        candidate = obs.get("tool_outputs") or {}
                                        if isinstance(candidate, dict) and set(candidate.keys()) & set(non_finish_call_ids):
                                            outputs = candidate

                            # Count all tool calls, including finish (matches original agentrllm JSON)
                            tool_total += len(all_calls)

                            # Only record segments that have any calls or aligned outputs
                            if all_calls or outputs:
                                seg = {"tool_calls": all_calls, "tool_outputs": outputs}
                                transcript.append(seg)

                        tool_infos.append({"tool_calls_total": tool_total, "transcript": transcript})
                        transcripts_list.append(transcript)
                        tool_calls_total_list.append(tool_total)
                        # Capture env metadata from the final step if present
                        try:
                            st_last = steps[-1]
                            info = getattr(st_last, "info", None)
                            meta = info.get("metadata") if isinstance(info, dict) else None
                        except Exception:
                            meta = None
                        env_metadata_list.append(meta if isinstance(meta, dict) else None)
                    except Exception:
                        tool_infos.append({"tool_calls_total": 0, "transcript": []})
                        transcripts_list.append([])
                        tool_calls_total_list.append(0)
                        env_metadata_list.append(None)
        ...
        if len(tool_infos) == sample_count:
            non_tensors["tool"] = np.array(tool_infos, dtype=object)
            non_tensors["transcript"] = np.array(transcripts_list, dtype=object)
            non_tensors["tool_calls_total"] = np.array(tool_calls_total_list)
        if len(env_metadata_list) == sample_count:
            non_tensors["metadata"] = np.array(env_metadata_list, dtype=object)
