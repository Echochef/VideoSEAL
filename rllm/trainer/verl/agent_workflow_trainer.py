...
class AgentWorkflowPPOTrainer(RayPPOTrainer):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    # Dump rollout generations (inputs/outputs/scores) if enabled
                    ...
                    if rollout_data_dir:
                        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
                            try:
                                ...
                                # Prefer token_level_scores if present; otherwise fall back to traj/step rewards
                                if "token_level_scores" in batch.batch:
                                    token_level = batch.batch["token_level_scores"]
                                elif "traj_rewards" in batch.batch:
                                    token_level = batch.batch["traj_rewards"]
                                else:
                                    token_level = batch.batch.get(
                                        "step_rewards",
                                        batch.batch["responses"].new_zeros(batch.batch["responses"].shape),
                                    )

                                scores = token_level.sum(-1).cpu().tolist()

                                # Extra infos for debugging/inspection
                                extra: dict[str, list] = {}
                                for key in [
                                    "episode_ids",
                                    "trajectory_ids",
                                    ...
                                    if key in batch.non_tensor_batch:
                                        ...
                                        try:
                                            extra[key] = val.tolist() if hasattr(val, "tolist") else list(val)
                                        except Exception:
                                            extra[key] = [str(x) for x in val]

                                # Also pass per-row tool transcripts so entries carry multi-turn tool info
                                tool_infos = batch.non_tensor_batch.get("tool", None)
                                if tool_infos is not None:
                                    try:
                                        tool_infos_list = tool_infos.tolist() if hasattr(tool_infos, "tolist") else list(tool_infos)
                                    except Exception:
                                        tool_infos_list = tool_infos
                                    transcripts, calls_total = [], []
                                    for ti in tool_infos_list:
                                        if isinstance(ti, dict):
                                            transcripts.append(ti.get("transcript"))
                                            try:
                                                calls_total.append(int(ti.get("tool_calls_total", 0) or 0))
                                            except Exception:
                                                calls_total.append(0)
                                        else:
                                            transcripts.append(None)
                                            calls_total.append(0)
                                    extra["transcript"] = transcripts
                                    extra["tool_calls_total"] = calls_total

                                # Attach VLM/env metadata fields if present in non_tensor_batch
                                try:
                                    n = len(inputs)
                                    if "metadata" in batch.non_tensor_batch:
                                        val = batch.non_tensor_batch["metadata"]
                                        try:
                                            extra["metadata"] = val.tolist() if hasattr(val, "tolist") else list(val)
                                        except Exception:
                                            pass
                                    for k, v in list(batch.non_tensor_batch.items()):
                                        if not (str(k).startswith("vlm_") or k in ("reward_total_vlm", "reward_base", "vlm_factor")):
                                            continue
                                        try:
                                            lst = v.tolist() if hasattr(v, "tolist") else list(v)
                                            if len(lst) == n:
                                                extra[str(k)] = lst
                                        except Exception:
                                            continue
                                ...
                                # Dump JSONL using RayPPOTrainer helper (gts not used for workflows)
                                ...
                                # Also dump old-style JSON array: {step}.json for agentrllm compatibility
                                try:
                                    import json as _json
                                    import os as _os

                                    from rllm.rollout_json import normalize_tool_info as _normalize_tool_info

                                    filename_old = _os.path.join(rollout_data_dir, f"{self.global_steps}.json")

                                    # Prefer precomputed tool transcripts if available
                                    if tool_infos is None:
                                        tool_infos_list = [
                                            {"tool_calls_total": 0, "transcript": []} for _ in range(len(inputs))
                                        ]
                                    else:
                                        try:
                                            tool_infos_list = tool_infos.tolist() if hasattr(tool_infos, "tolist") else list(tool_infos)
                                    ...
                                    # Preload per-row metadata and flattened VLM fields if present
                                    metas = None
                                    try:
                                        if "metadata" in batch.non_tensor_batch:
                                            m = batch.non_tensor_batch["metadata"]
                                            metas = m.tolist() if hasattr(m, "tolist") else list(m)
                                    except Exception:
                                        metas = None
                                    flat_vlm: dict[str, list] = {}
                                    for k, v in list(getattr(batch, "non_tensor_batch", {}).items()):
                                        if not (str(k).startswith("vlm_") or k in ("reward_total_vlm", "reward_base", "vlm_factor")):
                                            continue
                                        try:
                                            flat_vlm[k] = v.tolist() if hasattr(v, "tolist") else list(v)
                                        except Exception:
                                            pass
                                    ...
                                    for i in range(len(inputs)):
                                        raw_tool_info = (
                                            tool_infos_list[i]
                                            if i < len(tool_infos_list) and isinstance(tool_infos_list[i], dict)
                                            else {"tool_calls_total": 0, "transcript": []}
                                        )
                                        norm_tool = _normalize_tool_info(raw_tool_info) or raw_tool_info
                                        row: dict[str, object] = {
                                            "input": inputs[i],
                                            "output": outputs[i],
                                            "score": float(scores[i]) if i < len(scores) else 0.0,
                                            "reward": float(scores[i]) if i < len(scores) else 0.0,
                                            "step": int(self.global_steps),
                                            "tool": norm_tool,
                                        }
                                        if metas and i < len(metas):
                                            row["metadata"] = metas[i]
                                        for k, lst in flat_vlm.items():
                                            try:
                                                if i < len(lst):
                                                    row[str(k)] = lst[i]
                                            except Exception:
                                    ...
                                    with open(filename_old, "w", encoding="utf-8") as f:
                                        _json.dump(items, f, ensure_ascii=False, indent=2)
                                    print(f"Dumped old-format generations to {filename_old}")
                                except Exception as ee:
                                    print(f"[warn] Failed to dump old-format generations: {ee}")
                            except Exception as e:
                                print(f"[warn] Failed to dump rollout generations: {e}")
    ...
    def _validate_agent(self):
        ...
        dump_inputs: list[str] = []
        dump_outputs: list[str] = []
        dump_scores: list[float] = []
        dump_tool_infos: list[dict] = []
        dump_extra: dict[str, list] = defaultdict(list)
        dump_vlm: dict[str, list] = {}
        n_val_samples = int(self.config.actor_rollout_ref.rollout.val_kwargs.n)
        ...
        for test_data in self.val_dataloader:
            ...
            # Only keep last-step rows for each episode so metrics/dumps reflect final answers.
            try:
                if "is_last_step" in test_batch.non_tensor_batch:
                    is_last = np.asarray(test_batch.non_tensor_batch["is_last_step"])
                    last_idxs = np.where(is_last == True)[0]  # noqa: E712
                    if last_idxs.size > 0:
                        test_batch = test_batch.select_idxs(last_idxs.tolist())
            except Exception:
                pass
            ...
            # Optional: dump validation rollouts if configured (keep parity with rollout_data_dir behavior).
            if val_data_dir:
                try:
                    offset = len(dump_inputs)
                    inputs = self.tokenizer.batch_decode(test_batch.batch["prompts"], skip_special_tokens=True)
                    outputs = self.tokenizer.batch_decode(test_batch.batch["responses"], skip_special_tokens=True)
                    if "token_level_scores" in test_batch.batch:
                        token_level = test_batch.batch["token_level_scores"]
                    elif "traj_rewards" in test_batch.batch:
                        token_level = test_batch.batch["traj_rewards"]
                    else:
                        token_level = test_batch.batch.get(
                            "step_rewards",
                            test_batch.batch["responses"].new_zeros(test_batch.batch["responses"].shape),
                        )
                    scores = token_level.sum(-1).cpu().tolist()

                    n = len(inputs)
                    dump_inputs.extend(inputs)
                    dump_outputs.extend(outputs)
                    dump_scores.extend([float(x) for x in scores])
                    ...
                    for key in [
                        ...
                        if key in test_batch.non_tensor_batch:
                            ...
                            if len(lst) == n:
                                dump_extra[key].extend(lst)

                    tool_infos = test_batch.non_tensor_batch.get("tool", None)
                    if tool_infos is None:
                        tool_infos_list = [{"tool_calls_total": 0, "transcript": []} for _ in range(n)]
                    else:
                        try:
                            tool_infos_list = tool_infos.tolist() if hasattr(tool_infos, "tolist") else list(tool_infos)
                        except Exception:
                            tool_infos_list = tool_infos
                    if len(tool_infos_list) == n:
                        dump_tool_infos.extend(tool_infos_list)
                        transcripts, calls_total = [], []
                        for ti in tool_infos_list:
                            if isinstance(ti, dict):
                                transcripts.append(ti.get("transcript"))
                                ...
                                transcripts.append(None)
                                calls_total.append(0)
                        dump_extra["transcript"].extend(transcripts)
                        dump_extra["tool_calls_total"].extend(calls_total)

                    meta_lst = None
                    if "metadata" in test_batch.non_tensor_batch:
                        val = test_batch.non_tensor_batch["metadata"]
                        try:
                            meta_lst = val.tolist() if hasattr(val, "tolist") else list(val)
                        except Exception:
                            meta_lst = None
                    if not meta_lst or len(meta_lst) != n:
                        meta_lst = [None] * n
                    dump_extra["metadata"].extend(meta_lst)

                    present_vlm: dict[str, list] = {}
                    for k, v in list(test_batch.non_tensor_batch.items()):
                        if not (str(k).startswith("vlm_") or k in ("reward_total_vlm", "reward_base", "vlm_factor")):
                            continue
                        try:
                            lst = v.tolist() if hasattr(v, "tolist") else list(v)
                            ...
                            continue
                        if len(lst) == n:
                            present_vlm[str(k)] = lst

                    # Keep all VLM keys aligned with the global dump lists.
                    for k in list(dump_vlm.keys()):
                        if k not in present_vlm:
                            dump_vlm[k].extend([None] * n)
                    for k, lst in present_vlm.items():
                        if k not in dump_vlm:
                            dump_vlm[k] = [None] * offset
                        dump_vlm[k].extend(lst)
                except Exception as e:
                    print(f"[warn] Failed to collect validation rollout generations: {e}")
        ...
        if val_data_dir and dump_inputs:
            try:
                for k, lst in dump_vlm.items():
                    dump_extra[k] = lst

                # Dump JSONL using RayPPOTrainer helper
                self._dump_generations(
                    inputs=dump_inputs,
                    outputs=dump_outputs,
                    gts=[None] * len(dump_inputs),
                    scores=dump_scores,
                ...
                try:
                    ...
                    _os.makedirs(val_data_dir, exist_ok=True)
                    filename_old = _os.path.join(val_data_dir, f"{self.global_steps}.json")

                    metas = dump_extra.get("metadata", None)
                    flat_vlm = {
                        k: v
                        for k, v in dump_extra.items()
                        if str(k).startswith("vlm_") or k in ("reward_total_vlm", "reward_base", "vlm_factor")
                    }

                    items: list[dict[str, object]] = []
                    for i in range(len(dump_inputs)):
                        raw_tool_info = (
                            dump_tool_infos[i]
                            if i < len(dump_tool_infos) and isinstance(dump_tool_infos[i], dict)
                            else {"tool_calls_total": 0, "transcript": []}
                        )
                        norm_tool = _normalize_tool_info(raw_tool_info) or raw_tool_info
                        row: dict[str, object] = {
                            "input": dump_inputs[i],
                            "output": dump_outputs[i],
                            "score": float(dump_scores[i]) if i < len(dump_scores) else 0.0,
                            "reward": float(dump_scores[i]) if i < len(dump_scores) else 0.0,
                            "step": int(self.global_steps),
                            "tool": norm_tool,
                        }
                        if metas and i < len(metas):
                    ...
                    with open(filename_old, "w", encoding="utf-8") as f:
                        _json.dump(items, f, ensure_ascii=False, indent=2)
                    print(f"Dumped old-format validation generations to {filename_old}")
                except Exception as ee:
                    print(f"[warn] Failed to dump old-format validation generations: {ee}")
            except Exception as e:
                print(f"[warn] Failed to dump validation generations: {e}")
