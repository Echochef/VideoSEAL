...
class EpisodeLogger:
    ...
    def log_episode(self, episode: Episode, step: int, mode: str = "train", epoch: int = 0):
        ...
        # Best-effort: attach a time_reference block for downstream consumers
        try:
            tref = None
            task = episode.task if isinstance(episode.task, dict) else None
            if task is not None:
                # Direct field
                if "time_reference" in task and task["time_reference"] is not None:
                    tref = task["time_reference"]
                else:
                    # Nested under extra_info
                    extra = task.get("extra_info") if isinstance(task.get("extra_info"), dict) else None
                    if extra and "time_reference" in extra and extra["time_reference"] is not None:
                        tref = extra["time_reference"]
                    else:
                        # Derive from meta_json.clue_intervals when present
                        meta_raw = task.get("meta_json") or (extra.get("meta_json") if isinstance(extra, dict) else None)
                        if meta_raw is not None:
                            ...
                            if isinstance(meta_obj, dict) and "clue_intervals" in meta_obj:
                                tref = meta_obj.get("clue_intervals")
            if tref is not None:
                episode_data["time_reference"] = tref
        except Exception:
