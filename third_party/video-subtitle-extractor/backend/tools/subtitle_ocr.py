...
def extract_subtitles(data, text_recogniser, img, raw_subtitle_file,
    ...
    # 语言过滤策略：
    # - en: 去除中文字符
    # - all: 不做任何字符级过滤（保留混合文本）
    # - 其他: 保留原样
    lang_opt = getattr(options, 'REC_CHAR_TYPE', None) or getattr(_config, 'REC_CHAR_TYPE', 'ch')
    if str(lang_opt).lower() == 'en':
    ...
    line = ''
    loss_list = []
    # 准备底部区域（字幕）矩形：优先使用调用方传入的 sub_area；否则按 VSF_BOTTOM_RATIO 构造底部区域
    frame_h, frame_w = (img.shape[0], img.shape[1]) if isinstance(img, np.ndarray) else (0, 0)
    if sub_area is not None:
        sub_rect = sub_area  # (ymin, ymax, xmin, xmax)
    else:
        r = max(0.0, min(1.0, float(getattr(_config, 'VSF_BOTTOM_RATIO', 0.25))))
        ymin_b = int(frame_h * (1.0 - r)) if frame_h > 0 else 0
        ymax_b = frame_h if frame_h > 0 else 0
        sub_rect = (ymin_b, ymax_b, 0, frame_w)
    ...
    for idx, (content, coordinate) in enumerate(zip(text_res, coordinates)):
        ...
        # 底部区域（字幕判断）
        sub_area_polygon = sub_area_to_polygon(sub_rect) if sub_rect is not None else None
        coordinate_polygon = coordinate_to_polygon(coordinate)
        is_in_sub = False
        if sub_area_polygon is not None:
            inter = sub_area_polygon.intersection(coordinate_polygon)
            if not inter.is_empty:
                overflow_area_rate = ((sub_area_polygon.area + coordinate_polygon.area - inter.area) / sub_area_polygon.area) - 1
                if overflow_area_rate <= options.SUB_AREA_DEVIATION_RATE:
                    is_in_sub = True

        # 写入逻辑：
        # - split_regions=False 则不区分区域，全部写入 raw_subtitle_file
        # - 否则：字幕区域→raw_subtitle_file；其余→raw_aux_file（若提供）
        split_regions = bool(int(os.getenv('VSF_SPLIT_REGIONS', '1')))
        if prob > options.DROP_SCORE:
            if not split_regions:
                ...
                if is_in_sub:
                    ...
                    if raw_aux_file is not None:
                        raw_aux_file.write(f'{str(data["i"]).zfill(8)}\t{coordinate}\t{text}\n')
        ...
        # 调试打印：显示每个候选文本的选择与过滤信息
        if getattr(_config, 'DEBUG_PRINT_OCR', False):
            try:
                raw_text = rec_res[idx][0] if isinstance(rec_res, list) and idx < len(rec_res) else text
                print(f"[OCR] f#{data['i']} selected={selected} prob={prob:.3f} or={overflow_area_rate:.3f} coord={coordinate} text='{text}' raw='{raw_text}'")
            except Exception:
                pass

    # 输出调试信息（用于中文/日韩语丢失场景）
    dump_debug_info(options, line, img, loss_list, ocr_loss_debug_path, sub_rect, data)
...
def ocr_task_consumer(ocr_queue, raw_subtitle_path, raw_aux_path, sub_area, video_path, options):
    """
    ...
    with open(raw_subtitle_path, mode='w+', encoding='utf-8') as raw_subtitle_file, \
         open(raw_aux_path, mode='w+', encoding='utf-8') if raw_aux_path else open(os.devnull, 'w') as raw_aux_file:
        while True:
            try:
                ...
                extract_subtitles(data, text_recogniser, frame, raw_subtitle_file, sub_area, options, dt_box,
                                  rec_res, ocr_loss_debug_path, raw_aux_file)
            except Exception as e:
                # 打印错误并跳过该帧，继续消费后续任务，避免整条流水线中断
                print(e)
                continue
...
def subtitle_extract_handler(task_queue, progress_queue, video_path, raw_subtitle_path, raw_aux_path, sub_area, options):
    """
    ...
    """
    # 删除缓存
    for p in (raw_subtitle_path, raw_aux_path):
        try:
    ...
    ocr_event_consumer_thread = Thread(target=ocr_task_consumer,
                                       args=(ocr_queue, raw_subtitle_path, raw_aux_path, sub_area, video_path, options,),
                                       daemon=True)
    # 开启消费者线程
...
def async_start(video_path, raw_subtitle_path, raw_aux_path, sub_area, options):
    """
    开始进程处理异步任务
    ...
    p = Process(target=subtitle_extract_handler,
                args=(task_queue, progress_queue, video_path, raw_subtitle_path, raw_aux_path, sub_area, SimpleNamespace(**options),))
    # 启动进程
    p.start()
