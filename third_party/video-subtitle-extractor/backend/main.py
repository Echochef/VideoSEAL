...
class SubtitleDetect:
    ...
    def __init__(self):
        ...
        # 避免paddle内部parse_args解析我们上层脚本参数，临时清空argv
        _orig_argv = sys.argv
        try:
            sys.argv = [sys.argv[0]]
...
class SubtitleExtractor:
    ...
    def __init__(self, vd_path, sub_area=None, output_dir=None, filter_by_area=True, rec_char_type_option=None):
        importlib.reload(config)
        # 线程锁
        self.lock = threading.RLock()
        # 用户指定的字幕区域位置
        self.sub_area = sub_area
        # 用户指定的输出目录（用于最终 SRT/TXT 输出）
        self.output_dir = output_dir
        # 是否在OCR阶段按字幕区域过滤文本
        self.filter_by_area = filter_by_area
        # OCR 选项层面的语言标识（仅用于字符过滤逻辑，不影响模型选择）。如 'all' 则保留全部字符
        self.rec_char_type_option = rec_char_type_option
        # 创建字幕检测对象（仅在需要时懒加载，避免命令行参数冲突）
        ...
        # 若未指定字幕区域，且允许默认使用底部区域，则自动将字幕区域设为底部 VSF_BOTTOM_RATIO 比例的矩形
        if self.sub_area is None and getattr(config, 'VSF_USE_BOTTOM_DEFAULT', False):
            ratio = max(0.0, min(1.0, float(getattr(config, 'VSF_BOTTOM_RATIO', 0.25))))
            ymin = int(self.frame_height * (1.0 - ratio))
        ...
        # 提取的原始字幕文本存储路径（底部区域 = 字幕）
        self.raw_subtitle_path = os.path.join(self.subtitle_output_dir, 'raw.txt')
        # 非底部区域文本（辅助信息）
        self.raw_aux_path = os.path.join(self.subtitle_output_dir, 'raw_aux.txt')
        # 自定义ocr对象
        self.ocr = None
        # 打印识别语言与识别模式
    ...
    def _get_output_srt_path(self):
        """
        计算最终 SRT 输出路径。如果配置了自定义输出目录，则输出到该目录；否则与视频同目录。
        """
        if self.output_dir:
            try:
                ...
                pass
            return os.path.join(self.output_dir, f"{self.vd_name}.srt")
        else:
            return os.path.join(os.path.splitext(self.video_path)[0] + '.srt')
    ...
    def run(self, interactive=True):
        ...
        if interactive and self.sub_area is None:
        ...
        if config.WORD_SEGMENTATION:
            reformat.execute(self._get_output_srt_path(), config.REC_CHAR_TYPE)
        ...
        if config.GENERATE_TXT:
            self.srt2txt(self._get_output_srt_path())
    ...
    def generate_subtitle_file(self):
        ...
        if not self.use_vsf:
            ...
            srt_filename = self._get_output_srt_path()
    ...
    def start_subtitle_ocr_async(self):
        # 确保在开启子进程前使用 'spawn'，避免 Linux 下 fork + CUDA 导致初始化失败
        try:
        ...
        # 若不按区域过滤，则传递 None 以保留所有识别框（VSF 仍基于 self.sub_area 工作）
        process, task_queue, progress_queue = subtitle_ocr.async_start(
            self.video_path,
            self.raw_subtitle_path,
            self.raw_aux_path,
            self.sub_area if self.filter_by_area else None,
            options={
                'REC_CHAR_TYPE': (self.rec_char_type_option or config.REC_CHAR_TYPE),
...
## 交互式入口已移除，使用 extract_srt() 进行无交互调用


def extract_srt(video_path: str,
                *,
                language: str = 'en',
                mode: str = 'auto',
                area: tuple | None = None,
                drop_score: float | None = None,
                sub_area_deviation_rate: float | None = None,
                word_segmentation: bool = True,
                generate_txt: bool = False,
                keep_empty_ts: bool = False,
                interface: str = 'English',
                vsf_use_bottom_default: bool = True,
                vsf_bottom_ratio: float = 0.25,
                output_dir: str | None = None,
                filter_by_area: bool = True,
                split_regions: bool = True) -> str:
    """
    封装的Linux无交互OCR入口：输入视频与参数，输出srt路径。

    参数:
    - video_path: 视频全路径
    - language: 识别语言代码，如 'en', 'ch', 'japan', 'korean', 'chinese_cht' 等
    - mode: 模式 'fast' | 'auto' | 'accurate'（默认'auto'）
    - area: 可选字幕区域 (ymin, ymax, xmin, xmax)，不提供则按频率抽帧
    - drop_score: 覆盖识别置信度阈值（不传维持默认）
    - sub_area_deviation_rate: 区域越界容忍比例（默认None=使用配置，可设0.05~0.2放宽）
    - word_segmentation: 是否进行英文分词/清洗（默认True）
    - generate_txt: 是否同时生成txt（默认False）
    - keep_empty_ts: 合并VSF时是否保留空时间轴（默认False）
    - interface: 界面语言，仅影响提示文案，默认 'English'
    - vsf_use_bottom_default: 未传 area 时，是否自动使用视频底部区域用于 VSF（默认 True）
    - vsf_bottom_ratio: 底部区域所占比例，默认 0.25（即底部 25%）
    ...
    # 写入settings.ini以驱动config的语言与模式选择
    repo_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    settings_path = os.path.join(repo_root, 'settings.ini')
    # 将外部语言标识映射到模型使用语言（'all' 或 'cn' 映射为 'ch'）
    lang_map = {
        'cn': 'ch', 'zh': 'ch', 'zh_cn': 'ch', 'zh-cn': 'ch',
        'all': 'ch',
    }
    models_language = lang_map.get(language.lower(), language)
    with open(settings_path, 'w', encoding='utf-8') as f:
        f.write('[DEFAULT]\n')
        f.write(f'Interface = {interface}\n')
        f.write(f'Language = {models_language}\n')
        f.write(f'Mode = {mode}\n')
    ...
    if drop_score is not None:
        _config.DROP_SCORE = float(drop_score)
    if sub_area_deviation_rate is not None:
        _config.SUB_AREA_DEVIATION_RATE = float(sub_area_deviation_rate)
    _config.WORD_SEGMENTATION = bool(word_segmentation)
    _config.GENERATE_TXT = bool(generate_txt)
    _config.DELETE_EMPTY_TIMESTAMP = not bool(keep_empty_ts)
    # 为了后续合并辅助文本，保留中间产物
    _config.DEBUG_NO_DELETE_CACHE = True

    # 根据需要动态决定 VSF 默认底部区域（在此预先计算 area，规避 __init__ 内部 reload 覆盖）
    eff_area = area
    if eff_area is None and bool(vsf_use_bottom_default):
        # 打开视频读取尺寸
        import cv2  # 局部导入以避免不必要的全局开销
        ...
        try:
            ...
            cap.release()
        r = max(0.0, min(1.0, float(vsf_bottom_ratio)))
        ymin = int(frame_height * (1.0 - r))
        ymax = frame_height
        xmin = 0
        xmax = frame_width
        eff_area = (ymin, ymax, xmin, xmax)

    # 同步到配置（非必要，仅便于日志/后续模块读取）
    _config.VSF_USE_BOTTOM_DEFAULT = bool(vsf_use_bottom_default)
    _config.VSF_BOTTOM_RATIO = float(vsf_bottom_ratio)

    # 构造并运行提取器（无交互）
    from backend.main import SubtitleExtractor  # 避免循环导入
    # 当外部 language 指定为 'all' 时，不对字符做语言过滤；传入 rec_char_type_option='all'
    rec_char_type_option = 'all' if language.lower() == 'all' else None
    se = SubtitleExtractor(abs_video, eff_area, output_dir=output_dir, filter_by_area=filter_by_area,
                           rec_char_type_option=rec_char_type_option)
    # 将“是否分区写入（字幕/辅助）”标记注入到 options，供下游使用
    # 通过环境变量向 subtitle_ocr 传递（简单起见）；也可改成文件或全局 config
    os.environ["VSF_SPLIT_REGIONS"] = "1" if split_regions else "0"

    se.run(interactive=False)

    # 返回最终 SRT 路径（考虑 output_dir 自定义）
    if output_dir:
        srt_path = os.path.join(os.path.abspath(output_dir), f"{Path(abs_video).stem}.srt")
    else:
        srt_path = os.path.splitext(abs_video)[0] + '.srt'

    # 复制底部/辅助原始文本到输出目录，并生成合并后的 ocr_captions.json（subtitle+aux）
    try:
        import shutil
        import cv2
        import pysrt
        ...
        # 推断临时 raw 路径
        tmp_dir = os.path.join(os.path.dirname(_config.BASE_DIR), 'output', Path(abs_video).stem, 'subtitle')
        raw_path = os.path.join(tmp_dir, 'raw.txt')
        raw_aux_path = os.path.join(tmp_dir, 'raw_aux.txt')
        out_dir = os.path.abspath(output_dir) if output_dir else os.path.dirname(srt_path)
        os.makedirs(out_dir, exist_ok=True)
        # 复制到稳定位置（与 srt 同目录）
        if os.path.exists(raw_path):
            shutil.copy2(raw_path, os.path.join(out_dir, Path(srt_path).stem + '.raw.txt'))
        if os.path.exists(raw_aux_path):
            shutil.copy2(raw_aux_path, os.path.join(out_dir, Path(srt_path).stem + '.raw_aux.txt'))

        # 构建合并 captions（subtitle + aux）
        subs = pysrt.open(srt_path, encoding='utf-8')
        # fps 用于将帧号映射到秒
        cap = cv2.VideoCapture(abs_video)
        fps = cap.get(cv2.CAP_PROP_FPS) or 25.0
        cap.release()
        # 读取辅助文本（帧号→文本）。当未分区时，回退到 raw.txt 作为窗口并入来源
        aux_items = []
        aux_file_copy = os.path.join(out_dir, Path(srt_path).stem + '.raw_aux.txt')
        source_for_window = aux_file_copy if os.path.exists(aux_file_copy) else os.path.join(out_dir, Path(srt_path).stem + '.raw.txt')
        if os.path.exists(source_for_window):
            with open(source_for_window, 'r', encoding='utf-8') as fa:
                for line in fa:
                    try:
                        ...
                        text = parts[2].strip()
                        ts_sec = frame_no / float(fps)
                        aux_items.append((ts_sec, text))
                    except Exception:
                        continue
        # 生成合并后的 captions.json
        fused: Dict[str, Any] = {}
        for i, sub in enumerate(subs, 1):
            start = max(0.0, float(sub.start.ordinal) / 1000.0)
            end = max(start, float(sub.end.ordinal) / 1000.0)
            key = f"{int(start)}_{int(end)}"
            sub_text = (sub.text or '').strip()
            # 收集落在窗口内的辅助文本（去重）
            aux_texts = []
            seen = set()
            for ts, tx in aux_items:
                if ts >= start and ts <= end:
                    tx1 = tx.strip()
                    if tx1 and tx1 not in seen:
                        seen.add(tx1)
                        aux_texts.append(tx1)
            if aux_texts:
                fused_text = f"subtitle: {sub_text}\naux: {' '.join(aux_texts)}"
            else:
                fused_text = sub_text
            fused[key] = {"caption": fused_text, "entities": []}
        fused["subject_registry"] = {}
        fused_path = os.path.join(out_dir, Path(srt_path).stem + '.ocr_captions.json')
        with open(fused_path, 'w', encoding='utf-8') as fo:
            fo.write(_json.dumps(fused, ensure_ascii=False, indent=2))
        print(f"[OK] fused ocr_captions.json saved to {fused_path}")
    except Exception as e:
        print(f"[WARN] build fused ocr_captions.json failed: {type(e).__name__}: {e}")
    return srt_path
