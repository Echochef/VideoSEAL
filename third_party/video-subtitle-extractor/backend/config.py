...
MODEL_VERSION = 'V4'
# 支持通过环境变量覆盖默认的模型根目录：
#   PADDLE_OCR_MODEL_ROOT=/path/to/models
# 该路径下应包含各版本子目录（例如 V4/ch_det、V4/ch_rec 等）
_model_root_env = os.getenv("PADDLE_OCR_MODEL_ROOT")
if _model_root_env:
    DET_MODEL_BASE = _model_root_env
    REC_MODEL_BASE = _model_root_env
else:
    # 默认为当前项目下的 models 目录
...
# --------------------- 请根据自己的实际情况改 start-----------------
# VSF 默认使用视频底部区域（比例）。若不希望自动使用底部区域，可改为 False
VSF_USE_BOTTOM_DEFAULT = True
# 默认使用视频底部 25% 作为 VSF 搜索字幕帧的区域
VSF_BOTTOM_RATIO = 0.25

# 是否生成TXT文本字幕
GENERATE_TXT = True

# 调试：是否打印每条OCR识别结果（可能大量输出）
